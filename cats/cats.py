    ...
    if len(typed_words) == 0 and len(source_words) == 0:
        ...
        for i in range(min(len(source_words), len(typed_words))):
            if typed_words[i] == source_words[i]:
                n += 1
        return n / len(typed_words) * 100.0
    # END PROBLEM 3
...
def feline_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    def count(t, s, n):
        type_len, source_len = len(t), len(s)
        if n > limit:
            return limit + 1
        elif type_len == 0 and source_len == 0:
            return n
        elif type_len == 0 or source_len == 0:
            return min(n + abs(type_len - source_len), limit + 1)
        elif t[0] == s[0]:
            return count(t[1:], s[1:], n)
        else:
            return count(t[1:], s[1:], n+1)
    return count(typed, source, 0)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    n = 0
    while n < len(typed) and typed[n] == prompt[n]:
        n += 1
